;; ----------------------------------------------------------------------
;; File: tempo-c-cpp.el - my abbrevs (template) for c/c++ programming
;; Created: 18 Jan 2008
;; Time-stamp: <Fri 2010-01-15 12:18 svarrette>
;;
;; Copyright (c) 2008 Sebastien Varrette <Sebastien.Varrette@uni.lu>
;;               http://varrette.gforge.uni.lu
;; $Id: tempo-c-cpp.el 624 2010-01-18 09:32:32Z svarrette $
;;
;; Version: 0.1
...
;  (local-set-key (read-kbd-macro "C-<return>")   'tempo-complete-tag)
  (local-set-key (read-kbd-macro "C-t C-t")      'tempo-complete-tag)
...
(tempo-define-template "c++-class"
                       '("// ==============================================================" > n>
                         ...
                         > n>
                         "/* Mutators */" > n>
                         "void set" (s fnBase) "(const " (s type) " & " (s var) ") { " (s m_var) " = " (s var) "; }" > n>
                         > n>
                         "};\t// ============ end class " (s class) " ==================="  > n>
                         )
                       "class"
...
(tempo-define-template "c++-namespace"
                       '(>"namespace " (p "namespace name :" name) " {" > n>
                          > ~ n>
                          "} // end namespace " (s name) > n>
                          )
                       "namespace"
                       "Insert a C++ namespace statement"
                       'c++-tempo-tags)
